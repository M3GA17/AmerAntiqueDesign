...
public class UpdatePhotoCommandHandler(IProductRepository productRepository) : ICommandHandler<UpdatePhotoCommand, Result>
{
    public async Task<Result> Handle(UpdatePhotoCommand command, CancellationToken cancellationToken)
    {
        ...
        product.UpdatePhoto(new IdProductPhoto(command.IdProductPhoto), command.Name);
